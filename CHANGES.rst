...
- Only configured arrs are run: a Sonarr-only (or Radarr-only) config now skips the other arr with a dim ledger note instead of failing its run with an "unexpected error" traceback every cycle, and explicitly selecting an unconfigured arr (``run single --radarr``) fails with a one-line error naming the missing keys. A half-configured arr (url without api_key, or vice versa) is skipped with a warning naming the missing key, since that's almost certainly a mistake. A run with nothing runnable fails fast, before the mapping sources are downloaded
- ``run single`` with no selection flags now runs every configured arr, mirroring scheduled mode (previously it printed a usage hint and failed); ``--sonarr``/``--radarr``/``--movie-id``/``--series-id`` narrow the run as before
- ``advanced.log_level`` now actually applies to CLI runs (the CLI always logged at INFO and only the programmatic path honored it), and both run commands accept ``--log-level`` to override it for that invocation (cli > config > INFO)
- New ``config validate`` command: reports whether config.yml parses and validates (listing the offending keys otherwise) plus what a run would use - per-arr configured status and whether qBittorrent credentials are set (preview mode otherwise). New ``config show`` command: prints the effective config with every default applied and secrets redacted - API keys, passwords, usernames, webhook URLs, every value in the free-form ``qbittorrent.options`` block, and any ``user:pass@`` login embedded in a URL - safe to paste into a bug report. Neither writes the starter template
- ``seadexarr --version``/``-V`` prints the installed version, ``-h`` works everywhere as ``--help``, a bare group command (e.g. ``seadexarr run``) prints its help instead of a bare "missing command" error, ``run single``'s boolean flags drop the noisy auto ``--no-*`` variants, and ``cache stats`` prints aligned rows instead of a single dense line
- Failed CLI commands now exit non-zero (previously always 0), including a ``run single`` where any arr run fails, and a missing cache/backup file or a malformed (unparseable-YAML) config is reported as a one-line message instead of a traceback. An unreachable arr or a rejected arr API key is reported as a clean one-line config error (like the existing qBittorrent one) naming the unreachable URL and the config keys to check - a Sonarr run that also contacts Radarr (``ignore_movies_in_radarr``) names both candidates instead of blaming Sonarr for a Radarr outage
...
- An invalid ``SCHEDULE_TIME`` falls back to the configured ``schedule.interval_hours`` with a report instead of crashing the scheduler
