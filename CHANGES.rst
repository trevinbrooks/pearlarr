...
- Unless private releases are allowed (``seadex.private_releases``), prefer public URLs per release group; if the only option for a release the Arr is missing is private, log a warning and skip the title (leaving it uncached so it's retried) instead of grabbing a private release
- **Breaking:** ``seadex.public_only`` is replaced by ``seadex.private_releases`` (``allow`` / ``warn`` / ``fallback``): ``allow`` is the old ``public_only: false``, ``warn`` (the default) is the old ``public_only: true``, and the new ``fallback`` grabs the entry's best public alternative when the preferred release is only available privately, warning only when no public alternative can be found
