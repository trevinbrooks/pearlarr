...
Report vulnerabilities through [GitHub's private vulnerability reporting](https://github.com/trevinbrooks/seadexarr/security/advisories/new) (the Security tab, "Report a vulnerability").
...
Log files, `seadexarr config show` output, and error text never contain API keys, passwords, webhook URLs, or logins embedded in URLs - at any log level.
This is a tested promise, so pasting logs or `config show` output into a bug report is safe by design.

## No telemetry

SeaDexArr sends nothing anywhere except the services you configure.
