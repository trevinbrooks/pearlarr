...
pydantic models in ``seadexarr/modules/config.py`` (plus enum member docstrings
and the env-var registry), and this script renders every other surface from
them:

- ``seadexarr/modules/config_sample.yml`` - the starter config template
...
from seadexarr.modules.paths import PROJECT_URL

REPO_ROOT = Path(__file__).resolve().parents[1]
CONFIG_SOURCE = "seadexarr/modules/config.py"
REGEN_COMMAND = "uv run python scripts/gen_docs.py"

SAMPLE_PATH = REPO_ROOT / "seadexarr" / "modules" / "config_sample.yml"
...
SCHEMA_URL = f"{RAW_BASE}/v{version('seadexarr')}/schemas/config.schema.json"
...
def stitch(document: str, island: str, content: str, source: str) -> str:
    ...
    if open_match is None or close_index < 0 or close_index < open_match.end():
        raise GenerationError(f"island {island} is missing or malformed in {CONFIGURATION_PATH.name}")
    opener = (
...
def render_configuration(groups: tuple[GroupDoc, ...]) -> str:
    ...
    document = stitch(document, "env-vars", render_env_table(), "seadexarr/modules/env_registry.py")
    for group in groups:
