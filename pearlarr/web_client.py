...
`arr_http.make_httpx_client`: the arr client stays pinned to
...
transient-retry policy (retry count, jittered backoff, retryable-status set)
for those scrapes.
"""
...
def make_web_client() -> httpx.Client:
    ...
    - Client-level timeout of (5, 30) (connect, read), so no call site can
      forget one.
...
def get_with_retries(
    ...
    *,
    ...
    the exponential backoff, capped. Returns the final response: a success, a
    terminal non-retryable status, or the still-transient one once retries run
    out - the call site's `raise_for_status` surfaces a bad status.
    """
