...
This guide explains how to run Pearlarr in production.
It covers the Docker image in detail, bare-metal scheduling, what lives in the data directory, backups, upgrades, what an interruption does, and how to uninstall cleanly.
...
| `main` | Every push to `main`, the cutting edge that may break. |
...
The cache schema is versioned and migrates itself at load. No manual step is needed.
...
- **The run lock cannot wedge.**
  It is an OS-level advisory lock, released by the kernel when the process dies, so a crash never blocks future runs.
