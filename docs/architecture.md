...
| Placement | `placement_types`, `parse_reading`, `release_names`, `placer`, `window_placement`, `grab_placement`, `torrent_listings`, `listing_fold`, `video_files`, `sonarr_mapper`, `sonarr_parse` | mapping a release's files onto episodes at grab time and at import time, the series' SeaDex listings and size identities it uses, and the Sonarr `/parse` cache the placement reads |
...
| `graphql.anilist.co` | AniList titles, formats, and episode counts. Cached responses serve while the host is unreachable. |
...
- `pearlarr/cache.py` - a preview run never commits, so every staged write is discarded on close and preview mode can never mark a title as handled.
- `pearlarr/grab_pipeline.py` - an unparseable tracker never reaches TorrentService.add, whose raise is a defensive contract. This skip and warn enforces it: handing one through unwinds the id's url loop, dropping later grabbable releases.
- `pearlarr/import_quality.py` - the import payload always carries a quality key. Omitting it crashes Sonarr in FileNameBuilder.AddQualityTokens (observed on Sonarr 4.x).
...
- `pearlarr/probe_verdicts.py` - only a CLEAN importPending defers to Sonarr. A warning/error-flagged one is a failed Sonarr import attempt it won't reliably retry, so waiting would burn the readiness deadline.
