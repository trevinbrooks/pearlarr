...
- `pearlarr/sonarr_import_plan.py` - only a CLEAN importPending defers to Sonarr - a warning/error-flagged one is a failed Sonarr import attempt it won't reliably retry, so waiting would burn the readiness deadline.
