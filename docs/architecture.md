...
| Placement | `placement_types`, `parse_reading`, `release_names`, `placer`, `grab_placement`, `sonarr_mapper`, `sonarr_parse` | mapping a release's files onto episodes at grab time and at import time, and the Sonarr `/parse` cache the placement reads |
