...
The run summary lists a title under "needs action" with "numbering does not match its SeaDex listing; import it by hand", and the release's "skipped" line names the mismatch. The release is a specials pack whose `S00Exx` numbers follow another numbering than the specials SeaDex lists it for, such as an older TVDB numbering or TMDB's specials order: it holds as many files as the listed specials, and at least one of its numbers is not a listed special. Importing such a pack by its own numbers would put files on the wrong episodes. When the AniBridge mappings pair its numbers one to one with the listed specials, Pearlarr grabs the pack and imports each file onto its paired special, and no row appears. If an older version already imported part of the pack by its own numbers, onto specials the pairing disagrees with, the pairing is not used for the rest. The pack gets this row instead, and Pearlarr does not grab it, when one of its numbers has no pairing, the pairings do not match the listed specials one to one, a pairing lands on a special another entry of the show also maps, the entries listing the pack pair against different TMDB shows, a file's title or size points at a different special than its pairing, or another Sonarr series the pack is recommended for places one of its files by its number. Download it and import it by hand, matching each file to its episode by content. The row stops once the pack is in place: every special of the title holds one of the pack's files at its listed size, or a file of another recommended release covering that special. A special the pack has no file for keeps the row.
