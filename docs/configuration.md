...
| `ready_timeout` | `600` |  | Seconds to then wait for Sonarr to rescan and import the finished files. Measured from the last imported file, not the whole import: a season pack landing file-by-file only times out after this long with no new file. |
