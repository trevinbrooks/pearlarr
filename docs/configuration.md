...
| `ready_timeout` | `600` |  | Seconds to then wait for Sonarr to rescan and import the finished files. Measured from the last file Pearlarr saw land rather than the download's completion, so a season pack imported file-by-file times out only after this long with no visible progress. |
