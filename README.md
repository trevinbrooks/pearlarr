...
Pearlarr keeps the anime in your [Sonarr](https://sonarr.tv) and [Radarr](https://radarr.video)
...
![A Pearlarr grab notification in Discord](example_post.png)
...
6. **Import** (optional, Sonarr) — Pearlarr waits for the downloads to finish and shepherds them
...
`pearlarr` service into an existing stack), create the config directory yourself, and bring it
up:
...
The image also carries a healthcheck (`pearlarr paths`), so `docker ps` shows the container
...
[supercronic](https://github.com/aptible/supercronic) runs `pearlarr run single` on the
`PEARLARR_CRON` schedule (default `0 */6 * * *`, i.e. every 6 hours), evaluated in the `TZ`
timezone (default UTC). `PEARLARR_RUN_ON_START=true` (the default) also runs one catch-up pass
...
`pearlarr run scheduled` loop.

Overlapping runs are double-guarded: supercronic won't start a run while the previous one is still
going, and Pearlarr holds its own cross-process file lock besides. If you enable a blocking
...
the container uses; if you loosen it (e.g. `chmod 644`), Pearlarr warns on every run that the
file — which holds API keys — is readable by other users.

### pip

Pearlarr needs Python 3.13 or newer:

```
pip install pearlarr
```
...
pearlarr run single     # one pass, or just `pearlarr` for the scheduled loop
```

Or install the cutting edge from source:

```
git clone https://github.com/trevinbrooks/pearlarr.git
cd pearlarr
pip install -e .
```
...
Pearlarr keeps everything — `config.yml`, the caches (`cache.db`, `mappings.db`) and `logs/` —
...
Override the location with the `PEARLARR_DATA_DIR` environment variable or the global
`--data-dir` flag (the flag wins). Run `pearlarr paths` to print the resolved locations. The
Docker image sets `PEARLARR_DATA_DIR=/config`, so the `/config` volume mount holds the whole data
directory.

## CLI

Every command answers `--help` (or `-h`), and `pearlarr --version` prints the installed version.
...
for your shell is available via `pearlarr --install-completion`.

In Docker, run commands through your compose service: `docker compose run --rm pearlarr <args>`.

### `pearlarr run`

`run scheduled` — the default when you invoke bare `pearlarr` — loops forever, running every
...
schedules via supercronic + `PEARLARR_CRON` instead (see [Scheduling](#scheduling)).
...
confirming where Pearlarr is reading and writing, especially with a custom `--data-dir` or
`PEARLARR_DATA_DIR`.
...
Commands that modify the cache refuse to run while a Pearlarr run is active in the same data
directory, so they can't clobber an in-flight database.

## How a release is chosen

A SeaDex entry usually lists several torrents. Pearlarr cuts them down in order:
...
`advanced.max_torrents_to_add`; with `advanced.interactive` on, Pearlarr prompts you to choose
...
Two modes control how Pearlarr decides you already have a release:
...
to the torrent hashes Pearlarr itself grabbed before (from its cache). Updated releases are
...
- **Arr-side changes** (`advanced.detect_arr_activity`, on by default): each run polls the arr's
  ...
  (Pearlarr stopped for longer than that) re-checks everything once.
- **Config changes**: the cache is *not* invalidated when the config changes. After editing
  ...
  `pearlarr cache remove` so the next pass re-evaluates everything.

## Waiting for imports (Sonarr)

Pearlarr can optionally wait for grabbed torrents to finish downloading and then shepherd them
into Sonarr (Sonarr only; on Radarr runs this is a no-op). After a torrent is added, Pearlarr
...
Pearlarr's own authoritative episode mapping) when Sonarr can't auto-import them or isn't
...
defaults. `pearlarr config init` writes a commented starter file, and `pearlarr config show`
...
Pearlarr verifies TLS against the **operating-system trust store** (via
...
- `qbittorrent.host` / `qbittorrent.username` / `qbittorrent.password` — connection details for
  qBittorrent. All three are needed to add torrents; leave any blank and Pearlarr runs in
...
- `imports.post_import_category` — qBittorrent category to move a torrent to once its import is
  ...
  created if it doesn't exist; the move happens when Pearlarr verifies the import, so it needs a
...
- `schedule.interval_hours` — hours between cycles of the bare-metal `run scheduled` loop,
  re-read each cycle. The Docker image ignores it (`PEARLARR_CRON` owns the container cadence).
  Defaults to 6
...
- `advanced.detect_arr_activity` — re-check titles whose files Sonarr/Radarr changed since the
  ...
  you deliberately replace releases arr-side and don't want Pearlarr re-evaluating them.
  Defaults to true
...
`pearlarr run single`, and non-zero exit codes make the commands scriptable as-is. The same
...
```python
from pearlarr import RunDeps, RunLoop, RunServices, SonarrSync, setup_logger
from pearlarr.modules.config import AppConfig, Arr
from pearlarr.modules.mappings import MappingResolver, MappingSources
from pearlarr.modules.paths import ensure_data_dir, resolve_paths
from pearlarr.modules.web_client import make_web_client
...
`AppConfig.load` writes the starter template to the data directory (run `pearlarr paths` to find
...
Pearlarr is a fork of [seadexarr](https://github.com/bbtufty/seadexarr), originally created by
[bbtufty](https://github.com/bbtufty). Release data comes from
