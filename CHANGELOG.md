...
- A file named with an episode range, such as `S01E05-06`, imports onto every episode of the range, not just the first one Sonarr reads, and the grab counts it as covering the whole range. A range reaching past the episodes SeaDex lists the release for is left for a hand import instead of going onto its first episode. When an episode of a multi-episode file already holds a copy of that same file, Sonarr now replaces the copy with the one file instead of keeping both.
...
- A release's own numbering is read from more name shapes: an `S02E01` key, a `- 01 - Title` middle number, a packed `S0101`, and a trailing number after the title. A run numbered from the middle of the series (a split cour's `12..23`) fills a window as wide as it is.
- A whole-franchise pack, where several `1..N` runs fit the entry's window, imports the run the entry's AniList title names, judged by the words that title adds to the series title, and never a run Sonarr already read whole into another season. A one-episode entry with several unnumbered files in the batch (a movie beside its sequel, an OVA beside its preview) imports the file the title names the same way, never a preview or an opening, and never a file that shares only the franchise's name in another language with the title. Pending records now carry the series and AniList titles, so the import reads the same tie-break as the grab.
- A file name with a season token and no episode number (`S2 - OVA`), which Sonarr matches to every episode of that season, now imports as the one-episode entry it belongs to when that season is not the entry's. A file whose Sonarr match resolves outside the entry no longer blocks an unnumbered file from taking the entry's last episode.
- A release numbered `1..N` for a season of N episodes, which Sonarr matched partly onto that season's specials, imports as the season's own numbering. An entry holding an episode TVDB files as a special, such as a recap between two episodes, takes a release run in the series' absolute order. A whole-season release listed on one cour's entry imports that cour's slice and leaves the other cour's files alone. A whole-season release is not sliced for a specials entry or for a season whose numbering has a gap. One Sonarr read, even in part, into another season is neither sliced nor imported file by file as Sonarr read it. A release Sonarr read half into a season and half onto the specials, when the series lists as many specials as the release has files, is not imported as Sonarr read it either: only its own numbering, over an entry as wide as it, places it.
- A release carrying two versions of a file (`- 09` and `- 09v2`, the `v2` trailing or before the title) imports the later version and sets the earlier one aside as a duplicate. Release numbers framed by an `Episode` word or followed by the absolute number in brackets (`S2 - 01 [13] - Title`) are read.
...
- A release whose file names carry no episode number for the entry's episodes, while its other seasons' names do, is grabbed instead of being recorded as already owned, and no longer lets a sibling release duplicate its on-disk copy. The first run after upgrading re-checks every cached title once and says so.
...
- Release selection applies the audio preference before the best tag. Previously the audio preference only ranked releases within the best set, so an owned dual-audio copy listed on the entry was replaced by a single-audio best.
- A change to the selection rules in code now re-checks cached titles on the next run, the same way a change to the `seadex` settings does.
