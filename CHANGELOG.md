...
- Movies whose year parses as an episode number ("Chronicle.2020" read as S20E20) now import to the entry's sole resolved episode when that parsed episode provably does not exist in the series.
- All-numberless specials batches ("Special 1".."Special N") now import in order when the file count exactly matches the entry's remaining episodes. Any numbered, seeded, or ambiguous file in the batch keeps the whole-batch refusal.
- An extra whose bare-season name Sonarr matches to a whole season ("Show S05 Ending.mkv") is no longer pre-assigned every episode of that season at grab time - the shape that could import one OP/ED video as all twelve episodes. Grab-time assignments now follow the same limits as import-time assignment: a match spanning more than three episodes, a half-resolving multi-episode span, and a second file claiming an already-assigned episode (a v2 next to its v1) are all left for import time, which refuses ambiguity instead of guessing.
- Full-season parses are now refused at grab time by Sonarr's own `fullSeason` flag rather than only the more-than-three-episode span cap, closing the gap where a bare-season extra Sonarr matched to a season of three or fewer episodes slipped under the cap and was pre-assigned the whole season (import-time assignment already refused these).
- Checksum and sidecar files (`.blake3`, `.md5`, `.sha256`, `.mks` subtitle tracks, `.tif`, `.m3u8`) are no longer treated as importable videos, so a release shipping them no longer sticks on "intended file missing" warnings until the import deadline.
- A torrent carrying the same filename in two folders no longer warns that the file "could not be matched to an episode" after that same name was imported: the warning now reports only names that truly went unplaced, each once.
