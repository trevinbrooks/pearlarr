...
- A torrent imported across several passes (some files by Sonarr, the rest by Pearlarr) no longer lingers in Sonarr's queue afterwards, where completed-download handling could one day re-import it. Sonarr clears an entry itself only when a single import covers the grab's full episode count; once every entry sharing the torrent has imported, Pearlarr now removes the leftover entry - recorded by Sonarr as manually ignored, the torrent keeps seeding. New `imports.remove_from_queue` (default on) turns this off.
