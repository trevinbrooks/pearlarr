...
Everything lives in one data directory (`seadexarr paths` prints it).
...
| Artifact | Source |
| --- | --- |
| `seadexarr/modules/config_sample.yml` | attribute docstrings in `seadexarr/modules/config.py` |
| `schemas/config.schema.json` | the same config models |
| `docs/configuration.md` (islands between `gen:` markers) | config models + `seadexarr/modules/env_registry.py` |
| `CONTRIBUTING.md` (the env-var island below) | `seadexarr/modules/env_registry.py` |
...
| An environment variable | Register it in `seadexarr/modules/env_registry.py`, then regenerate |
...
1. Add the field to its group model in `seadexarr/modules/config.py`, with an
...
1. Add the display name to the right `*_TRACKER_NAMES` tuple in
   `seadexarr/modules/config.py` (a parity test pins the set against the seadex
   library's `Tracker` enum).
2. If its releases carry a parseable download page, add a parser to the table
   in `seadexarr/modules/torrents.py` (`PARSEABLE_TRACKERS` derives from it) —
...
<!-- gen:env-vars - GENERATED by scripts/gen_docs.py from seadexarr/modules/env_registry.py; do not edit between the markers; regenerate: uv run python scripts/gen_docs.py -->
| Variable | Read by | Meaning |
| --- | --- | --- |
| `SEADEXARR_DATA_DIR` | application | Override the data directory; the global `--data-dir` flag wins over it. |
| `SEADEXARR_CRON` | Docker entrypoint | Cron schedule for the container's recurring runs. |
| `SEADEXARR_RUN_ON_START` | Docker entrypoint | Whether the container runs a catch-up pass at start, before the cron cadence takes over. |
<!-- /gen:env-vars -->

Names use the `SEADEXARR_` prefix with `__` as the nesting delimiter (a future
settings layer would read `SEADEXARR_SONARR__URL` into `sonarr.url`), so new
names must stay unambiguous under that split. Register new variables in
`seadexarr/modules/env_registry.py`.
